...
function buildApiUrl(symbol) {
    ...
    var baseUrl = plasmoid.configuration.apiUrl || 'https://api.polygon.io/v2'
    
    // Polygon.io API example - can be adapted for other providers
    if (baseUrl.indexOf('polygon.io') !== -1) {
        return baseUrl + '/aggs/ticker/' + symbol + '/prev?adjusted=true&apikey=' + apiKey
    }
    
    // Alpha Vantage API example
    if (baseUrl.indexOf('alphavantage.co') !== -1) {
        return 'https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=' + symbol + '&apikey=' + apiKey
    }
    
    // Yahoo Finance fallback (unofficial, may be less reliable)
...
function buildCandlestickUrl(symbol, timeframe) {
    ...
    // Default to 1-day timeframe for intraday data
    var timespan = getTimespanFromTimeframe(timeframe)
    var multiplier = getMultiplierFromTimeframe(timeframe)
    
    if (baseUrl.indexOf('polygon.io') !== -1) {
        return baseUrl + '/aggs/ticker/' + symbol + '/range/' + multiplier + '/' + timespan + '/2024-01-01/2024-12-31?adjusted=true&sort=asc&apikey=' + apiKey
    }
    
    // Alpha Vantage intraday data
    if (baseUrl.indexOf('alphavantage.co') !== -1) {
        var interval = getAlphaVantageInterval(timeframe)
...
function getStockProvider() {
    var apiUrl = plasmoid.configuration.apiUrl || ''
    
    if (apiUrl.indexOf('polygon.io') !== -1) return 'polygon'
    if (apiUrl.indexOf('alphavantage.co') !== -1) return 'alphavantage'
    return 'yahoo'
}

function parseStockResponse(jsonString, symbol) {
    var provider = getStockProvider()
    
    switch (provider) {
        case 'polygon':
            return parsePolygonQuoteResponse(jsonString, symbol)
        case 'alphavantage':
            return parseAlphaVantageQuoteResponse(jsonString, symbol)
        default:
